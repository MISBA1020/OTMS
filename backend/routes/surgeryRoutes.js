...
router.post('/', async (req, res) => {
    try {
        ...
        if (overlapping) {
            return res.status(400).json({ message: 'Time slot overlaps with an existing surgery in this OT.' });
        }
        ...
        if (new Date(startTime) <= now && new Date(endTime) > now) {
            await OperationTheatre.findByIdAndUpdate(operationTheatreId, { status: 'In Use' });
        }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (updated.status === 'Completed' || updated.status === 'Cancelled') {
            await OperationTheatre.findByIdAndUpdate(updated.operationTheatreId, { status: 'Available' });
        }
