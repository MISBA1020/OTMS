    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"><QrCode className="w-5 h-5 text-indigo-500" /> Set QR — {set.setId}</h3>
        ...
        <div ref={printRef} className="p-6 space-y-4">
          ...
          <div className={`border-2 rounded-2xl p-4 ${isExpired ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-gray-50'}`}>
            ...
            <div className="grid grid-cols-2 gap-1 text-xs text-gray-600 mb-3">
              ...
              <span className={isExpired ? 'text-red-600 font-bold' : ''}><b>Expiry:</b> {set.expiryDate ? format(new Date(set.expiryDate), 'dd/MM/yyyy') : 'N/A'}</span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-7 space-y-6">
          {/* Section 1: Identity */}
          <div>
            <h3 className="text-sm font-black text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2"><Package className="w-4 h-4" /> Set Identity</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Zone</label>
                <select required className={inputCls} value={form.zone} onChange={e => set('zone', e.target.value)}>
                  {ZONES.map(z => <option key={z}>{z}</option>)}
                </select>
              </div>
              <div>
                <label className={labelCls}>Instrument Set Name</label>
                <input required className={inputCls} placeholder="e.g. Laparotomy Set" value={form.instrumentName} onChange={e => set('instrumentName', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Instrument Count</label>
                <input required type="number" min="1" className={inputCls} placeholder="12" value={form.instrumentCount} onChange={e => set('instrumentCount', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Batch Number</label>
                <input required className={inputCls} placeholder="BATCH-2026-001" value={form.batchNumber} onChange={e => set('batchNumber', e.target.value)} />
              </div>
            </div>
          </div>
          ...
          {/* Section 2: Process */}
          <div>
            <h3 className="text-sm font-black text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2"><FlaskConical className="w-4 h-4" /> Process Details</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Sterilization Method</label>
                <select required className={inputCls} value={form.sterilizationMethod} onChange={e => set('sterilizationMethod', e.target.value)}>
                  {METHODS.map(m => <option key={m}>{m}</option>)}
                </select>
              </div>
              <div>
                <label className={labelCls}>Cycle Number</label>
                <input className={inputCls} placeholder="e.g. C-042" value={form.cycleNumber} onChange={e => set('cycleNumber', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Temperature</label>
                <input className={inputCls} placeholder="134°C" value={form.temperature} onChange={e => set('temperature', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Pressure</label>
                <input className={inputCls} placeholder="2.1 bar" value={form.pressure} onChange={e => set('pressure', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Duration (minutes)</label>
                <input type="number" min="1" className={inputCls} placeholder="20" value={form.duration} onChange={e => set('duration', e.target.value)} />
              </div>
              <div>
          ...
          <div>
            <h3 className="text-sm font-black text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2"><ShieldCheck className="w-4 h-4" /> Dates & Staff</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>Sterilization Date & Time</label>
                <input required type="datetime-local" className={inputCls} value={form.sterilizedDate} onChange={e => set('sterilizedDate', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Expiry Date</label>
                <input required type="date" className={inputCls} value={form.expiryDate} onChange={e => set('expiryDate', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Sterilized By (Technician)</label>
                <input required className={inputCls} placeholder="Staff name" value={form.sterilizedBy} onChange={e => set('sterilizedBy', e.target.value)} />
              </div>
              <div>
                <label className={labelCls}>Checked / Verified By</label>
                <input required className={inputCls} placeholder="Supervisor / Nurse name" value={form.checkedBy} onChange={e => set('checkedBy', e.target.value)} />
              </div>
            </div>
            <div className="mt-4">
              <label className={labelCls}>Notes (optional)</label>
              <textarea className={`${inputCls} resize-none h-20`} placeholder="Any additional remarks..." value={form.notes} onChange={e => set('notes', e.target.value)} />
            </div>
          </div>

          {error && <p className="text-sm font-bold text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 py-3">{error}</p>}
        </form>
        ...
        <div className="px-7 py-5 border-t border-gray-100 flex justify-end gap-3 shrink-0">
          ...
          <button form="" onClick={handleSubmit} disabled={loading} className="px-8 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-500/20 transition-all disabled:opacity-60">
...
export default function Sterilization() {
  ...
  const summary = { total: sets.length, sterilized: sets.filter(s => s.status === 'Sterilized').length, expired: sets.filter(s => s.status === 'Expired' || (s.expiryDate && new Date(s.expiryDate) < new Date())).length };
      ...
      <div>
        ...
        {zoneSets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {zoneSets.map(s => (
              <SetCard key={s._id} set={s} onEdit={set => { setEditSet(set); setShowForm(true); }} onQR={setQrSet} onDelete={handleDelete} />
            ))}
          </div>
        )}
      </div>
