...
const mapSurgeryToFormData = (surgery) => ({
  uhid: surgery.uhid || "",
  aadharNo: surgery.aadharNo || "",
  ipNumber: surgery.ipNumber || "",
  patientName: surgery.patientName || "",
  age: surgery.age ?? "",
  sex: surgery.sex || "M",
  ward: surgery.ward || "FSW",
  diagnosis: surgery.diagnosis || "",
  surgery: surgery.surgery || "",
  surgeryCategory: surgery.surgeryCategory || "Major",
  priority: surgery.priority || "Planned",
  operationTheatreId:
    typeof surgery.operationTheatreId === "object"
      ? surgery.operationTheatreId?._id || ""
      : surgery.operationTheatreId || "",
  surgeonName: surgery.surgeonName || "",
  anesthesiologist: surgery.anesthesiologist || "",
  anaesthesiaType: surgery.anaesthesiaType || "GA",
  scrubNurse: surgery.scrubNurse || "",
  otTechnician: surgery.otTechnician || "",
...
export default function ScheduleModal({
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && initialData) {
      setFormData(mapSurgeryToFormData(initialData));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "edit" && editSurgeryId) {
        await axios.put(
          `http://localhost:5000/api/surgeries/${editSurgeryId}`,
          {
            ...payload,
      ...
      alert(
        err.response?.data?.message || "Failed to save. Check for overlaps.",
      );
    }
  };

  const selectedOtLabel =
    typeof initialData?.operationTheatreId === "object"
      ? initialData.operationTheatreId?.name
      : null;
  const selectedOtMissing =
    !!formData.operationTheatreId &&
    !ots.some((ot) => ot._id === formData.operationTheatreId);
  ...
  const handleAddStaff = async (e) => {
    ...
    if (lists[role]?.includes(name))
      return alert(`${addStaffModal.title.replace("Add ", "")} already exists`);
    ...
    try {
      ...
      alert(
        err.response?.data?.message ||
          "Verification Error pushing to persistence index",
      );
    }
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                  ...
                  >
                    ...
                    {selectedOtMissing && (
                      <option value={formData.operationTheatreId}>
                        {selectedOtLabel || "Current Theatre"}
                      </option>
                    )}
                    {ots.map((ot) => (
        ...
        <div className="px-8 py-5 border-t border-gray-100 bg-gray-50 flex justify-end space-x-3 shrink-0">
          ...
          <button
            type="submit"
            form="surgeryForm"
            className="bg-secondary-500 hover:bg-secondary-600 text-white px-8 py-2.5 rounded-xl font-bold shadow-lg shadow-secondary-500/30 transition-all text-lg"
          >
            {mode === "edit" ? "Update & Reschedule" : "Save & Schedule Block"}
          </button>
        </div>
      </div>
